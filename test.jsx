...
// This code block tries to load config from 3 places:
// 1. __firebase_config (provided by the Canvas environment)
// 2. import.meta.env (Vite/Vercel environment variables)
// 3. Placeholder values (if nothing else is found)
...
// 1. Try to load from Canvas
try {
  if (typeof __firebase_config !== 'undefined') {
    firebaseConfig = JSON.parse(__firebase_config);
  }
// 2. Try to load from Vite/Vercel .env
...
const SubmissionSuccess = ({ data, setView }) => {
  ...
  const handlePrint = () => {
    ...
    // Increased delay to 2.5 seconds
    setTimeout(() => {
  ...
  return (
    <>
      <div className="max-w-4xl mx-auto p-4 sm:p-8 my-12 print:hidden">
                  ...
                  {filteredApplications.length === 0 ? (
                    ...
                    filteredApplications.map(app => (
                      <tr key={app.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowMrap text-sm font-medium">
                          <div className="flex space-x-2">
                            <button
...
const CheckStatus = ({ setView }) => {
  ...
  const handleDownloadAdmitCard = () => {
    ...
    const opt = {
      margin:       0.5, // Use 0.5in margin to match application
...
// REMOVED w-[8.5in] to allow for scaling
const AdmitCard = React.forwardRef(({ app }, ref) => (
  <div ref={ref} className="p-8 bg-white"> 
    <div className="border-4 border-cyan-800 p-1 bg-cyan-50">
      <div className="border-2 border-cyan-700 p-6">
        
        <div className="flex justify-between items-center pb-4 border-b-2 border-cyan-600">
          <div className="flex items-center">
            ...
            <span className="ml-3 text-3xl font-bold text-cyan-800">Ajmal Super 40</span>
          </div>
          <div className="text-right">
            <h1 className="text-3xl font-bold text-cyan-700">ADMIT CARD</h1>
            <p className="text-lg text-gray-700">Entrance Test {new Date(app.examDate).getFullYear()}</p>
          </div>
        </div>

        <div className="flex mt-6">
          <div className="w-3/4 pr-6 space-y-4">
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              <div><strong className="text-gray-600 text-sm block">Roll Number:</strong> <span className="text-xl font-bold">{app.rollNumber}</span></div>
              ...
              <div><strong className="text-gray-600 text-sm block">Candidate's Name:</strong> <span className="text-lg">{app.studentName}</span></div>
              <div><strong className="text-gray-600 text-sm block">Date of Birth:</strong> <span className="text-lg">{app.dob}</span></div>
              <div><strong className="text-gray-600 text-sm block">Father's Name:</strong> <span className="text-lg">{app.fatherName}</span></div>
          ...
          <div className="w-1/4 flex flex-col items-center space-y-4">
            ...
            {app.photoURL ? <img src={app.photoURL} alt="Photo" className="w-32 h-40 object-cover border-2 border-gray-300" crossOrigin="anonymous" /> : <div className="w-32 h-40 border-2 border-dashed flex items-center justify-center text-gray-500">No Photo</div>}
            {/* ADDED crossOrigin="anonymous" */}
            {app.signatureURL ? <img src={app.signatureURL} alt="Signature" className="w-32 h-16 object-contain border-2 border-gray-300" crossOrigin="anonymous" /> : <div className="w-32 h-16 border-2 border-dashed flex items-center justify-center text-gray-500">No Signature</div>}
          </div>
        </div>

        <div className="mt-6 border-t-2 border-cyan-600 pt-6">
          <h3 className="text-xl font-bold text-cyan-800 mb-4">Exam Details</h3>
          <div className="grid grid-cols-2 gap-x-6 gap-y-4">
            <div><strong className="text-gray-600 text-sm block">Exam Date:</strong> <span className="text-lg font-semibold">{app.examDate}</span></div>
            <div><strong className="text-gray-600 text-sm block">Exam Time:</strong> <span className="text-lg font-semibold">{app.examTime}</span></div>
            <div className="col-span-2"><strong className="text-gray-600 text-sm block">Exam Centre:</strong> <span className="text-lg font-semibold">{app.examCentre}</span></div>
            <div className="col-span-2"><strong className="text-gray-600 text-sm block">Venue Address:</strong> <span className="text-lg">{app.examCentre} - Full address will be updated here.</span></div>
          </div>
        </div>
        
        <div className="mt-6 border-t-2 border-cyan-600 pt-6">
          <h3 className="text-xl font-bold text-cyan-800 mb-4">Instructions for Candidate</h3>
          <ul className="list-decimal list-inside text-sm text-gray-700 space-y-1">
            ...
            <li>No candidate will be allowed entry without this admit card and a valid photo ID.</li>
            <li>Reach the exam centre at least 30 minutes before the reporting time.</li>
            <li>Electronic devices (phones, smart watches, calculators) are strictly prohibited.</li>
        ...
        <div className="mt-12 text-right">
          <div className="inline-block text-center">
            <div className="w-48 h-16 border border-gray-400 bg-gray-100 flex items-center justify-center text-gray-500 text-sm">(Signature of Authority)</div>
            <p className="font-semibold mt-2">Issuing Authority</p>
          </div>
        </div>
